...
class ToolRegistry:
    ...
    def execute(self, name, arguments):
        ...
        except (OSError, ValueError, PermissionError, subprocess.SubprocessError, TimeoutError) as exc: return ToolResult(False, str(exc))
    ...
    def read_file(self, a):
        path = self._path(a.get("path", "")); return ToolResult(True, path.read_text(encoding="utf-8")[:100_000])
    ...
    def search_web(self, a):
        ...
        request = urllib.request.Request(url, headers={"User-Agent": "BRUCE/0.2"})
    ...
    def system_info(self, _):
        return ToolResult(True, json.dumps({"os": platform.platform(), "python": platform.python_version(), "machine": platform.machine()}, indent=2))
    ...
    def take_screenshot(self, a):
        ...
        path = self._path(a.get("path", "screenshot.png")); path.parent.mkdir(parents=True, exist_ok=True)
