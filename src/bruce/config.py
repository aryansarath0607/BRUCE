...
class Settings:
    ...
    def from_env(cls) -> 'Settings':
        ...
        return cls(
            ...
            allowed_roots=roots,
            memory_path=Path(os.getenv('BRUCE_MEMORY_PATH', str(cls.memory_path))),
            ...
            voice_input_name=os.getenv('BRUCE_VOICE_INPUT_NAME', ''),
            voice_input_index=int(raw_index) if raw_index.isdigit() else None,
            wake_word=os.getenv('BRUCE_WAKE_WORD', 'bruce').strip().lower() or 'bruce',
    ...
    def json(self) -> str:
        return json.dumps({'provider': self.provider, 'model': self.model, 'wake_word': self.wake_word}, indent=2)
